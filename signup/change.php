	...
	include 'initSessionCode.php';?>
		<div id="main">
			...
			<div id="site_content">
				...
				<input type="hidden" name="phonenb" value="'.$_POST['phonenb'].'" />
				<input type="hidden" name="selectfunction" value="'.$_POST['selectfunction'].'" />
				<input type="hidden" name="selectnameFacultyBranch" value="'.$_POST['selectnameFacultyBranch'].'" />
				<input type="hidden" name="selectnameMajor" value="'.$_POST['selectnameMajor'].'" />
				<input type="hidden" name="selectnameYear" value="'.$_POST['selectnameYear'].'" />';
				if(isset($_POST['filenb'])){
				echo '<input type="hidden" name="filenb" value="'.$_POST['filenb'].'" />';
				}?>
				
				<table border="0" style="padding: 0px;margin: 0px;">
<?php 
if(isset($_POST['finsih'])){
if($_POST['selectfunction']==='pr'){
			$namelastname=$_POST['name'].' '.$_POST['lastname'];
			$password=$_POST['psw'];
			$phonenb=$_POST['phonenb'];
			$email=$_POST['email'];
			$facultybranch=$_POST['selectnameFacultyBranch'];
			$query = "select idFaculty from faculty where nameFaculty='$facultybranch';";
			$result = mysqli_query($con, $query);
			$line = mysqli_fetch_array($result, MYSQLI_ASSOC);
			$facultybranch=$line['idFaculty'];
			$query = "insert into user values(default,'$namelastname','$email','$password',$phonenb,$facultybranch,'pr');";
			...
			$id=$listsubject->list[$i]->idSubject;
			$query = "insert into professor values(default,$id,$idUser);";
			mysqli_query($con, $query);
			}
...
if($_POST['selectfunction']==='st'){
			...
			$line = mysqli_fetch_array($result, MYSQLI_ASSOC);
			$facultybranch=$line['idFaculty'];
			$query = "insert into user values(default,'$namelastname','$email','$password',$phonenb,$facultybranch,'st');";
			mysqli_query($con, $query);
			$query = "select idUser from user where email='$email';";
			$result = mysqli_query($con, $query);
			$line = mysqli_fetch_array($result, MYSQLI_ASSOC);
			...
			$namemajor=$_POST['selectnameMajor'];
			$query = "select idMajor from major where idFaculty='$facultybranch' and nameMajor='$namemajor';";
			$result = mysqli_query($con, $query);
			$line = mysqli_fetch_array($result, MYSQLI_ASSOC);
			$idmajor=$line['idMajor'];
			$year=$_POST['selectnameYear'];
			$query = "select idYear from year where year='$year' and idMajor='$idmajor';";
			$result = mysqli_query($con, $query);
			$line = mysqli_fetch_array($result, MYSQLI_ASSOC);
			$idyear=$line['idYear'];
			$filenumber=$_POST['filenb'];
			
			$query = "insert into student values(default,$filenumber,$idyear,$idUser,$idmajor);";
			mysqli_query($con, $query);
			unset($_SESSION['subject']);
			unset($_SESSION['code']);
			session_destroy();
